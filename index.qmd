...
bibliography: bilancio_miss.bib
...
Riscaricato in formato molto piu facile da gestire 
read_excel("data_in/bes_2003_2023/Indicatori_per_provincia_sesso_ed.2025.xlsx" )
 -->
...
# amb_df <- readRDS(fs::path(bes_data_in,  "amb_df.rds"))
# ben_econ_df <- readRDS(fs::path(bes_data_in, "ben_econ_df.rds"))
# ist_formaz_df <- readRDS(fs::path(bes_data_in, "ist_formaz_df.rds"))
# lav_df <- readRDS(fs::path(bes_data_in, "lav_df.rds"))
# patr_cult_df <- readRDS(fs::path(bes_data_in, "patr_cult_df.rds"))
# qual_serv_df <- readRDS(fs::path(bes_data_in, "qual_serv_df.rds"))
# rel_pol_df <- readRDS(fs::path(bes_data_in, "rel_pol_df.rds"))
# ric_innov_df <- readRDS(fs::path(bes_data_in, "ric_innov_df.rds"))
# sal_df <- readRDS(fs::path(bes_data_in, "sal_df.rds"))
# sic_df <- readRDS(fs::path(bes_data_in, "sic_df.rds"))
# 
# # Esplora indicatori disponibili
# # tabyl(indicatori_di_struttura$indicatore)

# Import full BES dataset per provincia e sesso ----
all_bes <- read_excel("data_in/bes_2003_2023/Indicatori_per_provincia_sesso_ed.2025.xlsx" , sheet = "Indicatori_per_provincia_sesso") |> 
  janitor::clean_names() 
...
Uno degli indicatori "spia" di potentziale fragilità sociale è la `percentuale di popolazione residente di 65+ anni in familglie unipersonali` (**RE-R Statistica, 2025**, con dati al 31.12.2023).
...
over65_unip_map <- over65_unip_sf |> 
  ...
  geom_sf(data = ER_PR_distretti_sf, fill = NA, color = blu_sc, linewidth = rel(0.5)) +
    labs(
    title    = "% Popolazione residente di 65 anni e oltre in famiglie unipersonali",
    subtitle = "Indicatore per Comune (Anno 2023)",
    caption  = glue::glue("Fonte: Regione Emilia-Romagna - Statistica (2025)"),
    x = "",    y = "") +
  theme_minimal() +
  theme(
    legend.position = "right",
...
In generale, l’analisi degli indicatori demografici 2002-2025, mostra come Parma abbia un profilo demografico distintivo rispetto alla media regionale e nazionale, con alcune implicazioni interessanti per la programmazione dei servizi sociali. In particolare, si evidenzia più contenuto il trend di invecchiamento e una dinamica migratoria più vivace che contribuisce a bilanciare il deficit di natalità generalizzato.
...
Da diversi anni, Istat ha sviluppato un approccio multidimensionale per misurare il **“Benessere equo e sostenibile” (Bes)**, che consente di tracciare alcune fondamentali dimensioni del benessere, ma anche potenziali fonti di disuguaglianza confrantate per diverse aggregazioni territoriali.  [@istatBesTerritoriEmilia2025]
...
Occorre rilevare che, anche a Parma, come in Italia, permane un divario retributivo di genere significativo (questo è un dato complessivo, che andrebbe chiaramente approfondito).
```{r}
...
Anche a Parma, che supera sistematicamente l'Italia nei tassi di occupazione, si riscontra un divario di genere marcato (circa 20 punti percentuali per adulti, 18 per giovani, nel 2024) che rispecchia l'andamento nazionale.  
```{r}
#| label: occup_data
...
bes_disoccup_mf_plot <- bes_disoccup  |>
  ...
  labs(
    title = "Tassi di mancata partecipazione al lavoro per genere - Anno 2024",
    subtitle = "Confronto territoriale",
    caption = "Fonte: Istat, BES (Benessere Equo e Sostenibile), 2025",
    x = "",
    y = ""
...
bes_servizi <-  all_bes |>
  dplyr::filter(dominio%in% c("Qualità dei servizi", "Ambiente", "Paesaggio e patrimonio culturale"
  )) |> 
  dplyr::filter(territorio %in% c("Parma", "Emilia-Romagna", "Italia")) |> 
  mutate(territorio = factor(territorio, levels = c("Parma", "Emilia-Romagna", "Italia"))) |>
...
Nella qualità dei servizi si evidenziano performance migliori a Parma rispetto alla media regionale per quasi tutti gli indicatori considerati, in particolare nella disponibilità di verde urbano. Sui posti-km offerti dal trasporto pubblico locale, Parma è sopra la regione, ma resta al di sotto della media nazionale.
...
Nel dominio ambiente si evidenzia una certa disomogeneità tra Parma e l'Emilia-Romagna (come nella dispersione idrica in cui la media regionale sarebbe più virtuosa) o nella concentrazione di polveri sottili (pur molto al di sotto della media nazionale). Parma mostra performance migliori della media regionale per quel che concerne l'impremeabilizzazione del suolo e la produzione di rifiuti urbani. 
